...
> **📄 Полный итоговый отчёт — [REPORT.md](REPORT.md)** (метод, все таблицы, выводы).
> **Итог:** дообученный Granite-278m обошёл специализированную kazakh-e5, а его гибрид
> с BM25 — **#1** на бенчмарке (0.814 > 0.808); прирост подтверждён на независимом
> OOD-бенчмарке (речи).
...
| # | модель | ALL |
|---|---|---|
| **1** | **Granite-278m-ft ⊕ BM25 (наш гибрид)** | **0.814** |
...
Тезис доказан и перевыполнен: модель без официального казахского, таргетированно
дообученная, **превосходит специализированную kazakh-e5** (dense 0.752 > 0.747), а её
гибрид с BM25 (**0.814**) — **#1 на бенчмарке**, выше референс-топа kazakh-e5⊕BM25 (0.808).
Прирост подтверждён на независимом OOD-бенчмарке (речи, все срезы значимо вверх).

**Независимая OOD-проверка** ([отчёт](results/eval_ood_278m_40k.md)): на **втором**
бенчмарке (RAG-Two-Pass-Retrieval-QAZ — официальные речи, другой домен, не в обучении)
прирост подтвердился — **все тиры значимо вверх**: ALL 0.428→0.507 (+0.079, p<0.001),
factoid/paraphrase/low_overlap все значимы. Прирост **обобщается**, а не подгонка под
первый бенчмарк.
