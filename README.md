...
дообучение сработало направленно — **vocabulary-gap 0.242 → 0.322 (+0.080, p<0.001)**,
но с **значимой регрессией на natural (0.876 → 0.833, p=0.015)** из-за overfitting на
малом наборе. По ALL — статистически без изменений (0.617 → 0.626, p=0.257). Вывод:
таргетинг семантики подтверждён; для чистого прироста нужен гибрид ⊕BM25 и/или
