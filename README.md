...
**Прорыв — база 278m** ([отчёт](results/eval_278m_ft_vs_zeroshot.md)): на тех же
~14.7K пар (без hard-neg, без полного 40K) —

| | zero-shot | fine-tuned | Δ | p |
|---|---|---|---|---|
| **ALL** | 0.673 | **0.714** | +0.041 | **0.003** |
| vocab-gap | 0.303 | 0.466 | +0.163 | <0.001 |
| natural | 0.923 | 0.916 | −0.007 | 0.319 |

**Первый значимый прирост по ALL.** Дообученный 278m (0.714) обошёл **все** Granite
и подобрался к kazakh-e5 (0.747) — разрыв всего 0.033, при этом natural почти не
просел. Дальше: 40K синтетики + hard-negatives (BM25 + казахский стеммер) — заявка
обойти kazakh-e5.
